...
class ShoppingListScalingTest {
    ...
    fun `scaledIngredientQuantity multiplies numeric quantities`() {
        assertEquals("100", scaledIngredientQuantity("200", 0.5))
        assertEquals("200", scaledIngredientQuantity("400", 0.5))
        assertEquals("500", scaledIngredientQuantity("200", 2.5))
    }

    @Test
    fun `scaledIngredientQuantity handles comma decimals`() {
        assertEquals("1", scaledIngredientQuantity("1,5", 2 / 3.0))
    }

    @Test
    fun `scaledIngredientQuantity non-numeric uses ceil scale prefix`() {
        assertEquals("pinch", scaledIngredientQuantity("pinch", 0.5))
        assertEquals("2× pinch", scaledIngredientQuantity("pinch", 1.1))
        assertEquals("3× pinch", scaledIngredientQuantity("pinch", 2.5))
    }

    @Test
    fun `scaledIngredientQuantity blank stays blank`() {
        assertEquals("", scaledIngredientQuantity("   ", 2.0))
    }
    ...
    fun `scaleForAssignment is 1 when no people set`() {
        ...
        assertEquals(1.0, scaleForAssignment(plan, a, doc))
    }
    ...
    fun `scaleForAssignment uses defaultPersons`() {
        ...
        val a = DayAssignment("monday", "id", "R", persons = null)
        val doc = RecipeDoc("R", servings = 4, ingredients = emptyList())
        assertEquals(0.5, scaleForAssignment(plan, a, doc))
    }
    ...
    fun `scaleForAssignment coerces zero servings to 1`() {
        ...
        assertEquals(4.0, scaleForAssignment(plan, a, doc))
    }
}
