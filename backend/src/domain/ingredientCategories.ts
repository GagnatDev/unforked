...
/** All store categories in store-walk order (produce first, checkout items last). */
...
export const SHOPPING_CATEGORY_ORDER: readonly ShoppingCategory[] = SHOPPING_CATEGORIES;

/**
 * Normalized ingredient name: the key for family category overrides and for
 * matching persisted items during meal-plan sync.
 */
export function normalizeIngredientName(name: string): string {
  return name.trim().toLowerCase();
}

// Keyword table, Norwegian + English. A keyword matches at a word start; short
// keywords (< 4 chars) must cover the whole word bar a plural-ish suffix, so
// "te" (tea) never fires inside "tomatoes" while "egg" still matches "eggs".
// The longest matching keyword wins ("coconut milk" beats "milk", "frozen peas"
// beats "peas"); ties fall to definition order, so compound terms and produce
// come first. Deliberately modest — family overrides fix any misfire for good.
...
/**
 * Best-guess store category for an ingredient name. Family overrides (keyed by
 * normalized name) win outright; otherwise the longest matching keyword decides;
